...
export default class NewsModal03 {
  constructor(elem) {
    this.elem = elem
    this.init()
  }
  ...
  init() {
    ...
    const modal = $('#js-modal03')
    const modalClose = $('#js-modal03Close')
    const modalOverlay = $('#js-modal03Overlay')
    const open = 'is-open'

    // swiper
    // const modalSwiper = new Swiper('#js-galleryModalSwiper', {
    const modalSwiper = new Swiper('#js-modal03Swiper', {
      // loop: true,
      // spaceBetween: 30,
      simulateTouch: false,
      // noSwiping: true,
    ...
    $('.gallery-content__item_03').on('click', function(e) {
      e.preventDefault()
