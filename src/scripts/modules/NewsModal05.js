...
export default class NewsModal05 {
  constructor(elem) {
    this.elem = elem
    this.init()
  }
  ...
  init() {
    ...
    const modal = $('#js-modal05')
    const modalClose = $('#js-modal05Close')
    const modalOverlay = $('#js-modal05Overlay')
    const open = 'is-open'

    // swiper
    // const modalSwiper = new Swiper('#js-galleryModalSwiper', {
    const modalSwiper = new Swiper('#js-modal05Swiper', {
      // loop: true,
      // spaceBetween: 30,
      simulateTouch: false,
      // noSwiping: true,
    ...
    $('.gallery-content__item_05').on('click', function(e) {
      e.preventDefault()
