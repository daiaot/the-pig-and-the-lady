...
export default class NewsModal02 {
  constructor(elem) {
    this.elem = elem
    this.init()
  }
  ...
  init() {
    ...
    const modal = $('#js-modal02')
    const modalClose = $('#js-modal02Close')
    const modalOverlay = $('#js-modal02Overlay')
    const open = 'is-open'

    // swiper
    // const modalSwiper = new Swiper('#js-galleryModalSwiper', {
    const modalSwiper = new Swiper('#js-modal02Swiper', {
      // loop: true,
      // spaceBetween: 30,
      simulateTouch: false,
      // noSwiping: true,
    ...
    $('.gallery-content__item_02').on('click', function(e) {
      e.preventDefault()
